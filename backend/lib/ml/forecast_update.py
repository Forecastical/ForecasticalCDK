...
def update_forecast(time_threshold, prediction_matrix, alpha=0.5):
    ...
    utilized_predictions = prediction_matrix[
        np.where(prediction_matrix[:, 0] >= time_threshold), 1
    ]
    utilized_predictions = utilized_predictions.flatten()
    utilized_times = prediction_matrix[
        np.where(prediction_matrix[:, 0] >= time_threshold), 0
    ]
